...
export class PublicApiDocsComponent {
    ...
    readonly initiateCurl = `curl --request POST http://localhost:8060/api/v1/payments/initiate \\
  ...
  -d '{
    ...
    readonly statusCurl = `curl --request GET http://localhost:8060/api/v1/payments/VOTRE_PAYMENT_ID \\
  -H "X-API-Key: VOTRE_CLE_API_FIDELITYPAY"`;

    readonly otpCurl = `curl --request POST http://localhost:8060/api/v1/payments/VOTRE_PAYMENT_ID/actions/otp \\
  -H "Content-Type: application/json" \\
  -H "X-API-Key: VOTRE_CLE_API_FIDELITYPAY" \\
  -d '{ "otp": "123456" }'`;
    ...
    async copy(value: string, section: string): Promise<void> {
        ...
        try {
            ...
            try {
                ...
                this.copyError.set('La copie automatique a échoué. Sélectionnez le bloc de code et copiez-le manuellement.');
            }
        }
    }
